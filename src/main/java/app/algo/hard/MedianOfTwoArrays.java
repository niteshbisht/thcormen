...
public class MedianOfTwoArrays {
  ...
  static float medianSingle(int arr[], int n) {
    if (n == 0) return -1;
    if (n % 2 == 0) return (float) ((double) (arr[n / 2] + arr[n / 2 - 1]) / 2);
    return arr[n / 2];
  }
  ...
  static float findMedianUtil(int A[], int N, int B[], int M) {
    ...
    if (N == 1) {
      ...
      // A[] = {1}, B[] = {5, 8, 10, 20, 30}
      if (M % 2 == 1) return MO2(B[M / 2], (int) MO3(A[0], B[M / 2 - 1], B[M / 2 + 1]));
    ...
    // If the smaller array has two elements
      ...
      return MO4(
          B[M / 2], B[M / 2 - 1], Math.max(A[0], B[M / 2 - 2]), Math.min(A[1], B[M / 2 + 1]));
    }
